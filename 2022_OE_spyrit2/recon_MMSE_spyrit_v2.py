# -*- coding: utf-8 -*-
"""
Created on Sat Jul 30 11:12:08 2022
...
from spyrit.former.model_Had_DCAN import * # "EM-Net" branch
...
#%% Spyrit v1.2
model_12 = DenoiCompNet(img_size, M, Mean/img_size, Cov/img_size**2, 0, N0, 0, 
                     H/img_size, denoi=DConvNet());
...
DC = Generalized_Orthogonal_Tikhonov(sigma_prior = Cov_perm, M = M, 
                                     N = img_size**2)
Denoi = ConvNet()
model_20 = DC_Net(Noise, Prep, DC, Denoi)
model_20.eval()                    # Mandantory when batchNorm is used
...
#%% Plot sample (all channels summed up)
rec_sum = Pmat.T @ had_all.sum(axis=1)[:M,] / img_size**2
rec_sum = rec_sum.reshape(img_size, img_size,-1)

plt.figure()
plt.imshow(rec_sum, cmap='gray')
plt.colorbar()
plt.title(f"All channels summed up, {M} meas")
...
# Subsample
had_sub = had_all[:M,:]
rec_pinv_dir = (Pmat.T @ had_sub)/img_size**2
rec_pinv_dir = np.reshape(rec_pinv_dir, (img_size, img_size, -1))

fig , axs = plt.subplots(1,3)
#
im0 = axs[0].imshow(rec_pinv_dir[:,:,wav_0], cmap='gray')
fig.colorbar(im0, ax=axs[0])
axs[0].set_title(f"All meas, channel = {wav_0}")
#
im1 = axs[1].imshow(rec_pinv_dir[:,:,wav_1], cmap='gray')
fig.colorbar(im1, ax=axs[1])
axs[1].set_title(f"All meas, channel = {wav_1}")
#
im2 = axs[2].imshow(rec_pinv_dir[:,:,wav_2], cmap='gray')
fig.colorbar(im2, ax=axs[2])
axs[2].set_title(f"All meas, channel = {wav_2}"
                 )
# for consistency with the other reconstructions below
rec_pinv_dir = np.moveaxis(rec_pinv_dir, -1, 0)
...
# Init Output
rec_pinv_net = np.zeros((meas.shape[1], img_size, img_size))
rec_mmse_20 = np.zeros((meas.shape[1], img_size, img_size))
rec_mmse_12  = np.zeros((meas.shape[1], img_size, img_size))
...
for wav in range(meas.shape[1]):    
    ...
    # Pseudo-inverse from Net
    rec_gpu = Pinv(m[...,0::2]-m[...,1::2],Forward)
    rec_cpu = rec_gpu.cpu().detach().numpy().squeeze()
    rec_cpu = np.reshape(rec_cpu,(img_size,img_size))
    rec_pinv_net[wav,:,:] = rec_cpu

    # print max of pseudo inverse
    print(f'Max direct: {rec_pinv_dir[wav,:,:].max()} | Max from net: {rec_pinv_net[wav,:,:].max()}')

    # MMSE
    model_20.PreP.N0 = rec_pinv_net[wav,:,:].max()# /!\ NOT WORKING
    rec_mmse_20_gpu = model_20.reconstruct_mmse(m)
    rec_mmse_20_gpu = (rec_mmse_20_gpu + 1) * model_20.PreP.N0/2
    rec_mmse_20_cpu = rec_mmse_20_gpu.cpu().detach().numpy().squeeze()
    rec_mmse_20[wav,:,:] = rec_mmse_20_cpu
    
    # Net
    model_12.N0 = rec_pinv_net[wav,:,:].max()
    rec_mmse_12_gpu = model_12.forward_reconstruct_mmse(m,1,1,img_size,img_size)
    rec_mmse_12_gpu = (rec_mmse_12_gpu + 1) * model_20.PreP.N0/2 
    rec_mmse_12_cpu = rec_mmse_12_gpu.cpu().detach().numpy().squeeze()
    rec_mmse_12[wav,:,:] = rec_mmse_12_cpu;
...
if save_root:
    ...
    np.save(full_path_pinv, rec_pinv_net)
    np.save(full_path_mmse, rec_mmse_20)
    np.save(full_path_net, rec_mmse_12)
    
#%%
imagesc(np.sum(rec_pinv_dir, axis = 0))
imagesc(np.sum(rec_mmse_12, axis = 0))
...
fig , axs = plt.subplots(1,3)
#
im0 = axs[0].imshow(rec_pinv_net[wav_0,:,:], cmap='gray')
fig.colorbar(im0, ax=axs[0])
axs[0].set_title(f"Pinv, channel = {wav_0}")
#
im1 = axs[1].imshow(rec_pinv_net[wav_1,:,:], cmap='gray')
fig.colorbar(im1, ax=axs[1])
axs[1].set_title(f"Pinv, channel = {wav_1}")
#
im2 = axs[2].imshow(rec_pinv_net[wav_2,:,:], cmap='gray')
fig.colorbar(im2, ax=axs[2])
...
fig , axs = plt.subplots(1,3)
#
im0 = axs[0].imshow(rec_mmse_20[wav_0,:,:], cmap='gray')
fig.colorbar(im0, ax=axs[0])
axs[0].set_title(f"MMSE 2.0, channel = {wav_0}")
#
im1 = axs[1].imshow(rec_mmse_20[wav_1,:,:], cmap='gray')
fig.colorbar(im1, ax=axs[1])
axs[1].set_title(f"MMSE 2.0, channel = {wav_1}")
#
im2 = axs[2].imshow(rec_mmse_20[wav_2,:,:], cmap='gray')
fig.colorbar(im2, ax=axs[2])
axs[2].set_title(f"MMSE 2.0, channel = {wav_2}")
...
fig , axs = plt.subplots(1,3)
#
im0 = axs[0].imshow(rec_mmse_12[wav_0,:,:], cmap='gray')
fig.colorbar(im0, ax=axs[0])
axs[0].set_title(f"MMSE 1.2, channel = {wav_0}")
#
im1 = axs[1].imshow(rec_mmse_12[wav_1,:,:], cmap='gray')
fig.colorbar(im1, ax=axs[1])
axs[1].set_title(f"MMSE 1.2, channel = {wav_1}")
#
im2 = axs[2].imshow(rec_mmse_12[wav_2,:,:], cmap='gray')
fig.colorbar(im2, ax=axs[2])
axs[2].set_title(f"MMSE 1.2, channel = {wav_2}")
...
fig , axs = plt.subplots(1,3)
#
im0 = axs[0].imshow(rec_pinv_net.sum(axis = 0), cmap='gray')
fig.colorbar(im0, ax=axs[0])
axs[0].set_title("Pinv, sum of all channels")
#
im1 = axs[1].imshow(rec_mmse_20.sum(axis = 0), cmap='gray')
fig.colorbar(im1, ax=axs[1])
axs[1].set_title("MMSE 2.0, sum of all channels")
#
im2 = axs[2].imshow(rec_mmse_12.sum(axis = 0), cmap='gray')
fig.colorbar(im2, ax=axs[2])
axs[2].set_title("MMSE 1.2, sum of all channels")
...
fig , axs = plt.subplots(1,3)
#
im0 = axs[0].imshow(rec_pinv_net[wav_1,:,:], cmap='gray')
fig.colorbar(im0, ax=axs[0])
...
axs[1].set_title(f"MMSE 2.0, channel = {wav_1}")
#
im2 = axs[2].imshow(rec_mmse_12[wav_1,:,:], cmap='gray')
fig.colorbar(im2, ax=axs[2])
axs[2].set_title(f"MMSE 1.2, channel = {wav_1}")
