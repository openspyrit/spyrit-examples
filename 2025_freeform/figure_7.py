...
# boosts
boost_s1 = 10*np.log10(mse_fh2 / mse_s1)
boost_rs  = 10*np.log10(mse_fh2 / mse_rs)
boost_mh2 = 10*np.log10(mse_fh2 / mse_mh2)
boost_h1  = 10*np.log10(mse_fh2 / mse_h1)
...
gain = 20*np.log10(data[0,ind_std]/data[1:,ind_std])
...
fbar_s1 = find_closest_abscissa(boost_s1,  f_mean_ref, gain[0,:])
...
# These values were computed in figure_6.py
sigma_dark = np.array([
        [19.06419074, 18.59970943, 16.18167311, 16.95223562], # FH2
        [29.58125051, 29.37583337, 17.28607546, 17.99641753], # RS
        [19.06419074, 18.59970943, 16.18167311, 16.95223562], # MH2  
        [24.02996294, 23.81448521, 16.70019888, 17.40761311], # H1
        [29.58125051, 29.37583337, 17.28607546, 17.99641753]  # S1
        ])
...
plt.semilogx(f_mean_ref, boost_s1,  label='S1',  linewidth=lw)

symbol = 'ovd^'
prop_cycle = plt.rcParams['axes.prop_cycle']
colors = prop_cycle.by_key()['color']
 
for i in range(len(fbar_s1)):
    plt.semilogx(fbar_ref[1,i], gain[0,i], symbol[i], color=colors[0])
    plt.semilogx(fbar_ref[2,i], gain[1,i], symbol[i], color=colors[1])
    plt.semilogx(fbar_ref[3,i], gain[2,i], symbol[i], color=colors[2])
    plt.semilogx(fbar_ref[4,i], gain[3,i], symbol[i], color=colors[3])
...
plt.ylabel('MSE gain w.r.t. FH2 (in dB)', fontsize=fs)
