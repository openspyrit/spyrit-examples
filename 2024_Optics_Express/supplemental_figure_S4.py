...
recon_folder = 'recon/supplemental_figure_S4/'    # reconstructed images
...
for i, label in enumerate(label_list):
    ...
    plt.imsave(recon_folder_full / f'original_{label}.png', x_plot[0, :, :], cmap='gray')
...
alpha = 50 # Poisson law parameter for noisy image acquisitions
...
# label_list = ['brain', 'dog', 'panther', 'box', 'bird', 'car']
nu_list = [10, 20, 20, 20, 20, 20] # noise levels for each label
...
with torch.no_grad():
    ...
    for ii, nu in enumerate(nu_list):
        ...
        filename = f'pinv_pnp_{label_list[ii]}_nu_{nu}.png'
        full_path = recon_folder_full / filename
...
with torch.no_grad():
    ...
    for ii, mu in enumerate(mu_list):
        ...
        filename = f'dpgd_pnp_{label_list[ii]}_mu_{mu}.png'
        full_path = recon_folder_full / filename
