...
from imagenet_dataset import NetImage
...
#
    trainset = \
          NetImage(root=opt.data_train_root, train = True, transform=transform)
    trainloader = \
        torch.utils.data.DataLoader(trainset, batch_size=opt.batch_size,shuffle=False)
    testset = \
            NetImage(root=opt.data_val_root, train = False, transform=transform)
    testloader = \
         torch.utils.data.DataLoader(testset, batch_size=opt.batch_size,shuffle=False)
...
#
    ...
    if opt.n_iter :
        if opt.poisson_dc:
            print("Poisson + DC")
            cov_new_root = opt.model_root+ "iter_1_Cov_{}x{}".format(opt.img_size, opt.img_size)+'.npy';
            my_new_cov_file = Path(cov_new_root) / (cov_new_root)           
            if my_new_cov_file.is_file():
                Cov  = np.load(my_new_cov_file_)
            else :
                H_1 = opt.img_size*H;
                Var = Cov2Var(Cov_had)
                Perm = Permutation_Matrix(Var);
                Pmatfull = np.dot(Perm, H_1);
                Ptot = matrix2conv(Pmatfull);
                Cov = Stat_net(dataloaders, cov_new_root, model, device, Ptot, save = True)
            denoi =  sn_dp_iteratif_2(opt.CR, opt.img_size, Cov_had,H, model.recon, n_iter = opt.n_iter)
            denoi.set_layers(model.recon, Cov)
        else:
            print("DC alone")
    ...
    if opt.n_iter:
        if opt.poisson_dc:
            recon_type+="_Iteratif_sc_{}".format(opt.n_iter)
        else:
