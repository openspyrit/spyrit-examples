...
#
    ...
    model = noiCompNet(opt.img_size, opt.CR, Mean_had, Cov_had, opt.net_arch, opt.N0, opt.sig, H , denoi=denoi_img)
...
#    denoi = denoi_img
    ...
    denoi =  NeumannNet(opt.CR, opt.img_size, Cov_had,H, denoi_img, iterations = opt.n_iter)
    denoi.nonlinear_op = model.recon;
    model = noiCompNet(opt.img_size, opt.CR, Mean_had, Cov_had, opt.net_arch, opt.N0, opt.sig, H , denoi=denoi)
    model.recon.Gramian.weight.requires_grad = False
    ...
    if opt.n_iter:
        recon_type+="_neumann_{}".format(opt.n_iter)
    ...
    title = opt.model_root + "U_net" + '_' + net_type[opt.net_arch]+train_type+recon_type+suffix    
    print(title)
   ...
   #- save training history
    ...
    train_path = opt.model_root+'TRAIN_'+net_type+train_type+recon_type+suffix+'.pkl'
