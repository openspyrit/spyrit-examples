...
if __name__ == "__main__":
    ...
    parser.add_argument("--M",          type=int,   default=1024,  help="Number of patterns")
    parser.add_argument("--subs",       type=str,   default="rect",  help="Among 'var','rect','rand'")
    ...
    parser.add_argument("--N0",         type=float, default=10,   help="Mean maximum total number of photons. Choose inf for no noise.")
    parser.add_argument("--stat_root",  type=str,   default="../../stat/stl10", help="Path to precomputed data")
    ...
    #==========================================================================
    FO = Forward_operator_Split_ft_had(Pmat, Perm, opt.img_size, opt.img_size)
    
    if opt.N0 == float('inf'):
        print('Noise: None')
        Noi = Acquisition(FO)
    else:
        print('Noise: Gaussian approximation to Poisson')
        Noi = Acquisition_Poisson_approx_Gauss(opt.N0, FO)
    
    Prep = Preprocess_Split_diag_poisson(opt.N0, opt.M, opt.img_size**2)
    ...
    if opt.N0 == float('inf'):
        train_type = 'no_noise'.format(opt.N0)
    else:
        train_type = 'N0_{:g}'.format(opt.N0) 
        
    #- training parameters
    suffix = 'N_{}_M_{}_epo_{}_lr_{}_sss_{}_sdr_{}_bs_{}_reg_{}_seed_{}'.format(\
           opt.img_size, opt.M, opt.num_epochs, opt.lr, opt.step_size,\
           opt.gamma, opt.batch_size, opt.reg, opt.seed)

    title = opt.model_root / f'{opt.arch}_{opt.denoi}_{opt.data}_{opt.subs}_{train_type}_{suffix}'    
    print(title)
    ...
    train_path = opt.model_root / f'TRAIN_{opt.arch}_{opt.denoi}_{opt.data}_{opt.subs}_{train_type}_{suffix}.pkl'
