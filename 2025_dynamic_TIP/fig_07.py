...
f_wh_X = torch.linalg.solve(H_dyn_in_X.T @ H_dyn_in_X + eta * norm_in_X ** 2 * D2_in_X, H_dyn_in_X.T @ y2_exp.T)
...
f_wf_X = torch.linalg.solve(H_dyn_in_X.T @ H_dyn_in_X + eta * norm_in_X ** 2 * D2_in_X, H_dyn_in_X.T @ y2_exp.T)
...
ax[0, 0].imshow(f_wide_stat, cmap='gray')
...
ax[1, 0].imshow(f_cmos_ref, cmap='gray')
...
if save_fig:
    ...
    plt.imsave(path_fig / f'fig07_a_static_amp{amp_max}_frame{frame_ref}.pdf', f_wide_stat, cmap='gray')
    ...
    # (d) CMOS reference
    plt.imsave(path_fig / f'fig07_d_cmos_amp{amp_max}_frame{frame_ref}.pdf', f_cmos_ref, cmap='gray')
