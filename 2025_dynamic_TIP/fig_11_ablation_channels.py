...
if strategy == 'bin':
    ...
    win_size = meas2_torch.shape[1] // n_wav
    y1_exp = torch.stack([
        meas2_torch[:, i * win_size : (i + 1) * win_size].mean(dim=1)
        for i in range(n_wav)
    ], dim=1).moveaxis(-1, 0)

    wav = torch.stack([
        wavelengths[i * win_size : (i + 1) * win_size].mean()
        for i in range(n_wav)
    ], dim=0).numpy()
    ...
    win_size = meas2_torch.shape[1] // n_wav
    indices = [i * win_size - win_size // 2 - 1 for i in range(1, n_wav + 1)]
...
if n_wav > 1:
    ...
    plot_hs(strategy, f_stat_np, wav, suptitle="Static Reconstruction - Spectral Bands", save_fig=save_fig, 
...
l = n + 2 * amp_max
translation = (0, 0)
frame_ref = 0 # frame ref in CMOS referential
n_ppg = 16
...
cond = s[0]/s[-1]
print(f'H_dyn spectrum is [{s[-1]:.2f}, {s[0]:.2f}] (condition number: {cond:.2e})')
norm = s[0]

s = torch.linalg.svdvals(H_dyn_in_X)
cond_in_X = s[0]/s[-1]
print(f'H_dyn_in_X spectrum is [{s[-1]:.2f}, {s[0]:.2f}] (condition number: {cond_in_X:.2e})')
norm_in_X = s[0]
...
if save_fig:
    ...
    params = {
        "eta": float(eta),
        "eta_in_X": float(eta_in_X),
        "norm": float(norm),
        "norm_in_X": float(norm_in_X),
        "condition_number_H_dyn": float(cond),
        "condition_number_H_dyn_in_X": float(cond_in_X),
    }

    with open(str(path_fig / f"reco_{warping}_params.json"), "w") as f:
...
if save_fig:
    ...
    video_path = path_fig / f'video_rec.mp4'
...
if save_fig:
    ...
    plt.imsave(path_fig / f'fig_a_static_amp{amp_max}_frame{frame_ref}.pdf', 
               blue_box(f_wide_stat, amp_max=amp_max))

    # (b) wh in X
    plt.imsave(path_fig / f'fig_b_wh_in_X_amp{amp_max}_frame{frame_ref}.pdf',
               blue_box(f_wh_X_np, amp_max=amp_max))

    # (c) wf in X
    plt.imsave(path_fig / f'fig_c_wf_in_X_amp{amp_max}_frame{frame_ref}.pdf',
               blue_box(f_wf_X_np, amp_max=amp_max))

    # (d) CMOS reference
    plt.imsave(path_fig / f'fig_d_cmos_amp{amp_max}_frame{frame_ref}.pdf', 
               blue_box(f_cmos_ref, amp_max=amp_max), cmap='gray')

    # (e) wh in X ext
    plt.imsave(path_fig / f'fig_e_wh_in_X_ext_amp{amp_max}_frame{frame_ref}.pdf',
               blue_box(f_wh_Xext_np, amp_max=amp_max))

    # (f) wf in X ext
    plt.imsave(path_fig / f'fig_f_wf_in_X_ext_amp{amp_max}_frame{frame_ref}.pdf',
