...
if __name__ == "__main__":
    ...
    parser.add_argument("--precompute_root", type=str, default="models/IMNET/", help="Path to precomputed data")
    ...
    parser.add_argument("--model_root",type=str, default='models/IMNET/', help="Path to model saving files")
    ...
    trainset = \
        torchvision.datasets.ImageNet(root=opt.data_root, split='train',  transform=transform)
    trainloader = \
        torch.utils.data.DataLoader(trainset, batch_size=opt.batch_size, shuffle=False)

    testset = \
        torchvision.datasets.ImageNet(root=opt.data_root, split='val',  transform=transform)
    testloader = \
    ...
    model, train_info = train_model(model, criterion, optimizer, scheduler, dataloaders, device, opt.model_root,\
                                    num_epochs=opt.num_epochs, disp=True, do_checkpoint=opt.checkpoint_interval)
    ...
    # - training parameters
    suffix = '_N_{}_M_{}_epo_{}_lr_{}_sss_{}_sdr_{}_bs_{}_reg_{}'.format(opt.img_size, opt.CR, opt.num_epochs,
                                                                         opt.lr, opt.step_size,
